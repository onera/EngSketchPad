...
public:
  ...
  extern "C" int  EG_makeTransform( ego context, const double *xform, ego *oform );
  ...
  extern "C" int  EG_makeGeometry(egObject *context, int oclass, int mtype,
                                   egObject *refGeom, const int *ints,
                                   const double *data_dot, egObject **geom);
  extern "C" int  EG_evaluatX(const egObject *geom, /*@null@*/ const double *param,
                                    double *result);
...
)
{
  ...
  egObject **_face_vertices = (egObject **) EG_alloc(n_face_vertices * sizeof(egObject *));
  ...
  for (int i = 0; i < nloop; i++) {
    ...
    for (int i_edge=0; i_edge<nedge; ++i_edge) {
      ...
      if (nnode==2) {
        if (senses[i_edge]==1) {
          _face_vertices[i_face_vtx] = nodes[0];
        }
        else {
          _face_vertices[i_face_vtx] = nodes[1];
        }
        i_face_vtx++;
      }
    }
...
extern "C" {
/**
 * \brief From interface informations defined at CAD edges,
 * compute CAD node matching
 */
int
EG_compute_node_matching
(
  egObject *body,
  int       n_pairs,
  int      *pairs,
  double   *hmatrix,
  int     **out_node_matching
)
{
  // > Compute bbox diag for tolerance
  double box[6];
  EG_getBoundingBox(body, box);
  double diag = sqrt((box[0] - box[3]) * (box[0] - box[3]) +
                     (box[1] - box[4]) * (box[1] - box[4]) +
                     (box[2] - box[5]) * (box[2] - box[5]));

  // > Get edges and nodes
  int       nnode, nedge;
  egObject **nodes, **edges;
  ...
  for (int i_pair=0; i_pair<n_pairs; ++i_pair) {
    ...
    if (nnode_src!=2) {
      printf("Edge %d seems to be degenerated (defined with %d vertices)", src, nnode_src);
      return EGADS_DEGEN;
    }
    if (nnode_tgt!=2) {
      printf("Edge %d seems to be degenerated (defined with %d vertices)", tgt, nnode_tgt);
      return EGADS_DEGEN;
    }
    ...
    for (int i_node_src=0; i_node_src<nnode_src; ++i_node_src) {
      ...
      double xyz_src[3];
      egObject *ref, **dum;
      EG_getTopology(nodes_src[i_node_src], &ref, &oclass, &ntype, xyz_src,
                    &ndum, &dum, &senses);
    ...
    if (nmatch!=2) { // Both vertices must match for edge pair
      printf("Edge %d and %d seems not to be match", src, tgt);
      return EGADS_NOTFOUND;
    }

    int src_ind_node0 = EG_indexBodyTopo(body, nodes_src[0]);
    int tgt_ind_node0 = EG_indexBodyTopo(body, nodes_tgt[0]);
    int src_ind_node1 = EG_indexBodyTopo(body, nodes_src[1]);
    int tgt_ind_node1 = EG_indexBodyTopo(body, nodes_tgt[1]);
    if (src_tgt_node_pairs[0]==0 && src_tgt_node_pairs[1]==1) {
      node_matching[src_ind_node0-1] = tgt_ind_node0;
      node_matching[src_ind_node1-1] = tgt_ind_node1;
    }
    else if (src_tgt_node_pairs[0]==1 && src_tgt_node_pairs[1]==0) {
      node_matching[src_ind_node0-1] = tgt_ind_node1;
      node_matching[src_ind_node1-1] = tgt_ind_node0;
    }
    else {
...
)
{
  ...
  // > Count total edge number
  int  n_edge = 0;
  for (int i_face=0; i_face<nface; ++i_face) {
    n_edge += _count_face_nedge(object, faces[i_face]);
  }

  // > Fill face edge connectivity
  int i_write = 0;
  int       *face_edge_idx = (int       *) EG_calloc(nface+1,sizeof(int));
  egObject **face_edge     = (egObject **) EG_alloc (n_edge* sizeof(egObject));
...
)
{
  ...
  int i_write_pair = 0;
  int *node_pairs_idx = (int *) EG_alloc((n_itrf+1) *sizeof(int)); node_pairs_idx[0]=0;
  int *node_pairs     = (int *) EG_alloc(face_edge_idx[nface]*sizeof(int));
  int *face_referencd = (int *) EG_calloc(nface, sizeof(int));
  ...
  for (int i_itrf=0; i_itrf<n_itrf; ++i_itrf) {
    ...
    for (int i_pair=pairs_idx[i_itrf]; i_pair<pairs_idx[i_itrf+1]; ++i_pair) {
      ...
      // > Count n_node in face (=n_edge if closed)
      ...
      if (n_face_nodes_src!=n_face_nodes_tgt) {
        ...
        face_referencd[tgt] = 1;
      }

      // > Store vertices
      egObject **src_face_vertices = NULL;
      egObject **tgt_face_vertices = NULL;
      _store_face_nodes(object, face_src, n_face_nodes_src, &src_face_vertices);
      _store_face_nodes(object, face_tgt, n_face_nodes_tgt, &tgt_face_vertices);
      ...
      for (int i_node_src=0; i_node_src<n_face_nodes_src; ++i_node_src) {
        ...
        int found_match = 0;
        for (int i_node_tgt=0; i_node_tgt<n_face_nodes_tgt; ++i_node_tgt) {
          double xyz_tgt[3];
...
)
{
  ...
  // > Mandatory to count edges via faces because edge can be referenced by multiple faces
  int n_edge_pairs = 0;
  for (int i_itrf=0; i_itrf<n_itrf; ++i_itrf) {
    ...
    for (int i_pair=face_pairs_idx[i_itrf]; i_pair<face_pairs_idx[i_itrf+1]; ++i_pair) {
      ...
      // > nedge in face
      int n_face_edges_src = face_edge_idx[src+1]-face_edge_idx[src];
      int n_face_edges_tgt = face_edge_idx[tgt+1]-face_edge_idx[tgt];
  ...
  int i_write_pair = 0;
  int *edge_pairs_idx  = (int *) EG_alloc((n_itrf+1)*sizeof(int)); edge_pairs_idx[0] = 0;
  int *edge_pairs      = (int *) EG_alloc(2*n_edge_pairs*sizeof(int));
  int *edge_pairs_sign = (int *) EG_alloc(  n_edge_pairs*sizeof(int));
  ...
  for (int i_itrf=0; i_itrf<n_itrf; ++i_itrf) {
    ...
    for (int i_pair=face_pairs_idx[i_itrf]; i_pair<face_pairs_idx[i_itrf+1]; ++i_pair) {
      ...
      // > Pair up face edges
      for (int i_src_edge=face_edge_idx[src]; i_src_edge<face_edge_idx[src+1]; ++i_src_edge) {
        int nnode_src;
        egObject **nodes_src;
        double limits_src[2];
        EG_getTopology(face_edge[i_src_edge], &geom, &oclass, &mtype, limits_src, &nnode_src,
                       &nodes_src, &senses);
        ...
        if (n_src_nodes_found<2) {
          printf("EGADS Error: Edge %d has nodes no paired through interface\n", EG_indexBodyTopo(object, face_edge[i_src_edge]));
          return EGADS_NOTFOUND;
        }
        ...
        for (int i_tgt_edge=face_edge_idx[tgt]; i_tgt_edge<face_edge_idx[tgt+1]; ++i_tgt_edge) {
          ...
          egObject **nodes_tgt;
          double limits_tgt[2];
          EG_getTopology(face_edge[i_tgt_edge], &geom, &oclass, &mtype, limits_tgt, &nnode_tgt,
                        &nodes_tgt, &senses);
...
)
{
  ...
  for (int iloop=0; iloop<nloop; ++iloop) {
    ...
    int       *loop_senses;
    egObject **loop_edges;
    EG_getTopology(loops[iloop], &loop_geom, &loop_oclass, &loop_mtype, NULL, &loop_nedge, &loop_edges, &loop_senses);
    if (debug_verbose==1) {
    ...
    for (int iedge=0; iedge<loop_nedge; ++iedge) {
      ...
      if (debug_verbose==1) {
        printf("\tiedge = %d :: ind edge = %d\n", iedge, ind_edge);
      }

      if (ind_edge==pairs[0]) { // If in edge graph, take opposite
        new_loop_edges [iedge] =  loop_edges [pairs[1]-1];
        new_loop_senses[iedge] = -loop_senses[pairs[1]-1];
        nnew_edge++;
        if (debug_verbose==1) {
          printf("\t\t --> take opposite iedge = %d\n", pairs[1]);
        }
        patch_to_per_patch[1][ind_edge-1] = -pairs[1]; //encode relative orientation
        ...
        if (edge_nnode!=2) {
          printf("Edge %d is degenerated (%d vertices)", ind_edge, edge_nnode);
          exit(EXIT_FAILURE);
        }

        double edge_range[4]; // [tmin, tmax, ?, ?]
        EG_getRange(loop_edges[iedge], edge_range, &pflag);
        if (debug_verbose==1) {
          printf("\t\t tmin / tmax = %f %f\n", edge_range[0], edge_range[1]);
        }

        /**
         * Find or create periodize edge vertices
         */
        egObject* new_edge_nodes[2];
        for (int inode=0; inode<edge_nnode; ++inode) {
          int ndum;
          egObject **dum;
          double xyz[3];
          egObject  *node_geom;
          EG_getTopology(edge_nodes[inode], &node_geom, &oclass, &mtype, xyz, &ndum, &dum, &senses);

          int ind_node = EG_indexBodyTopo(bodies[0], edge_nodes[inode]);
          if (debug_verbose==1) {
            printf("\t\tinode = %d :: ind_node = %d\n", inode, ind_node);
          }
          if (node_matching[ind_node-1]==0) { // not a periodic vertex, create new one applying transfo
            if (debug_verbose==1) {
            ...
            new_edge_nodes[inode] = new_nodes[nnew_node];
            // TODO: tester de add dans le model plutot que le context

            /**
             * EG_indexBodyTopo gives -1 (shouldn't when added to body)
             * --> for now we guess it
             */
            patch_to_per_patch[0][ind_node-1] = nnode+nnew_node+1;

            node_matching[ind_node-1] = -(nnew_node+1);
            nnew_node++;
          }
          else if (node_matching[ind_node-1]<0) {
            if (debug_verbose==1) {
              printf("\t\t --> Get new node %d\n", abs(node_matching[ind_node-1]));
            }
            new_edge_nodes[inode] = new_nodes[abs(node_matching[ind_node-1])-1];
          }
          else {
            if (debug_verbose==1) {
              printf("\t\t --> Get node %d\n", node_matching[ind_node-1]);
            }
            int ind_match_node = node_matching[ind_node-1];
            new_edge_nodes[inode] = nodes[ind_match_node-1];

            patch_to_per_patch[0][ind_node-1] = ind_match_node;

          }
        }

        /**
         * Try to copy edge with transformation in order to get geometry
         */
        egObject  *fake_new_edge = NULL;
        egObject  *fake_new_edge_geom = NULL;
        double     fake_new_edge_limit[2];
        int        fake_new_edge_nnode = 0;
        egObject **fake_new_edge_nodes = NULL;
        int       *fake_new_edge_senses = NULL;
        ...
        EG_makeTopology(context, fake_new_edge_geom, oclass, mtype, fake_new_edge_limit, 2, new_edge_nodes, NULL, &new_edges[nnew_edge]);

        new_loop_senses[iedge] = loop_senses[iedge];

        double new_edge_range[4]; // [tmin, tmax, ?, ?]
        EG_getRange(new_edges[nnew_edge], new_edge_range, &pflag);
        if (debug_verbose==1) {
          printf("\t\tnew tmin / tmax = %f %f\n", edge_range[0], edge_range[1]);
        }
    ...
    EG_makeTopology(context, loop_geom, loop_oclass, loop_mtype, NULL, loop_nedge, new_loop_edges, new_loop_senses, &new_loops[nnew_loop]);
    nnew_loop++;
  ...
  egObject *face_geom;
  int face_oclass, face_mtype, face_nloop;
  egObject **face_loops;
  int *face_senses;
  EG_getTopology(faces[0], &face_geom, &face_oclass, &face_mtype, NULL, &face_nloop, &face_loops, &face_senses);
  double face_range[4]; // [umin, umax, vmin, vmax]
  EG_getRange(faces[0], face_range, &pflag);
  if (debug_verbose==1) {
    printf("face_oclass = %d\n", face_oclass);
    printf("face_mtype = %d\n", face_mtype);
    printf("umin / umax = %f %f\n", face_range[0], face_range[1]);
    printf("vmin / vmax = %f %f\n", face_range[2], face_range[3]);
  }

  egObject  *fake_new_face = NULL;
  EG_copyObject(faces[0], eg_transform, &fake_new_face);
  egObject  *fake_new_face_geom = NULL;
  double     fake_new_face_limit[4];
  int        fake_new_face_nnode = 0;
  egObject **fake_new_face_nodes = NULL;
  int       *fake_new_face_senses = NULL;
  EG_getTopology(
    fake_new_face,
    &fake_new_face_geom,
    &oclass,
  ...
  patch_to_per_patch[2][0] = 2;


  // translation sur la geometrie de la face ? --> TODO
  EG_makeTopology(context, fake_new_face_geom, face_oclass, face_mtype, NULL, face_nloop, new_loops, face_senses, &new_face);
  double new_face_range[4]; // [umin, umax, vmin, vmax]
  EG_getRange(new_face, new_face_range, &pflag);
  // Face importée du STEP
  int new_face_oclass, new_face_mtype, new_face_nloop;
  egObject *new_face_geom = NULL;
  egObject **new_face_loops;
  int *new_face_senses;
  EG_getTopology(new_face, &new_face_geom, &new_face_oclass, &new_face_mtype, NULL, &new_face_nloop, &new_face_loops, &new_face_senses);
  if (debug_verbose==1) {
    printf("new_face_oclass = %d\n", new_face_oclass);
    printf("new_face_mtype = %d\n", new_face_mtype);
    printf("umin / umax = %f %f\n", new_face_range[0], new_face_range[1]);
    printf("vmin / vmax = %f %f\n", new_face_range[2], new_face_range[3]);
  }


  // reassemble shell
  egObject *shell_faces[2] = {faces[0], new_face};
  // egObject *shell_faces[2] = {new_face};

  EG_makeTopology(context, NULL, SHELL, OPEN, NULL, 2, shell_faces, NULL, &new_shell);
  ...
  EG_makeTopology(context, body_geom, body_oclass, body_mtype, NULL, 1, body_shells, NULL, &per_body);
...
)
{
  ...
  for (int iface=0; iface<nface; ++iface) {
    per_shell_faces[per_nface++] = faces[iface];
  }


  // > Go through CAD faces and periodize ones that are not on periodic interface
  for (int iface=0; iface<nface; ++iface) {
    ...
    EG_getTopology(faces[iface], &face_geom, &face_oclass, &face_mtype, NULL, &face_nloop, &face_loops, &face_senses);
    ...
    if (matching_face == -1) { // Periodize faces that are not in periodic interface
      ...
      for (int iloop=0; iloop<face_nloop; ++iloop) {
        ...
        int       *loop_senses;
        egObject **loop_edges;
        EG_getTopology(face_loops[iloop], &loop_geom, &loop_oclass, &loop_mtype, NULL, &loop_nedge, &loop_edges, &loop_senses);
        if (debug_verbose==1) {
          printf("\n");
          printf("\tiloop = %d :: nedge = %d\n", iloop, loop_nedge);
        }
        ...
        for (int iedge=0; iedge<loop_nedge; ++iedge) {
          ...
          EG_getPCurve(
            faces[iface],
            loop_edges[iedge],
            loop_senses[iedge], // attention au sens quand on doit permuter ??
            &edge_pcurve_mtype,
            &edge_pcurve_ivec,
            &edge_pcurve_rvec
          );
          ...
          if (edge_matching[ind_edge-1]==0) { // If in edge graph, take opposite
            ...
            edge_matching[ind_edge-1] = -(nnew_edge+1);

            nnew_edge++;
          }
          else if (edge_matching[ind_edge-1]<0) {
            if (debug_verbose==1) {
              printf("\t\t --> Get new edge %d\n", abs(edge_matching[ind_edge-1]));
            }
            new_loop_edges [iedge] = new_edges[abs(edge_matching[ind_edge-1])-1];
            new_loop_senses[iedge] = loop_senses[iedge];
          }
          else {
            if (debug_verbose==1) {
              printf("\t\t\t --> Get edge %d (rel sign = %d)\n",
                edge_matching     [ind_edge-1],
                edge_matching_sign[ind_edge-1]
              );
            }
        ...
        egObject  *fake_new_loop = NULL;
        EG_copyObject(loops[iloop], eg_transform, &fake_new_loop);
        egObject  *fake_new_loop_geom = NULL;
        double     fake_new_loop_limit[4];
        int        fake_new_loop_nedge = 0;
        egObject **fake_new_loop_edges = NULL;
        int       *fake_new_loop_senses = NULL;
        EG_getTopology(
          fake_new_loop,
          &fake_new_loop_geom,
          &oclass,
